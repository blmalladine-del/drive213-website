...
export function HomePageContent({ settings, featuredCars }: HomePageContentProps) {
          ...
          <Container className="relative z-20 flex min-h-[inherit] items-center py-16 md:py-0 md:!pl-4 lg:!pl-6">
            <div className="w-full md:py-28 lg:py-32 md:mr-auto">
              ...
              <div className="mt-16 flex flex-wrap gap-4">
                ...
                >
                  {t('hero.browseFleet', locale)} <ArrowRight className="h-5 w-5" />
                </Button>
                <a
            ...
            <div className="grid gap-14 md:grid-cols-2 md:gap-20 lg:gap-24">
              <div>
                <h2 className="text-5xl font-bold leading-[1.05] tracking-tight text-white text-balance lg:text-6xl xl:text-7xl">
                  {t('whatYouBook.heading1', locale)}<br />{t('whatYouBook.heading2', locale)}
                </h2>
                <p className="mt-5 max-w-sm text-base leading-relaxed tracking-wide text-white/50">
                  {t('whatYouBook.subtext', locale)}
                </p>
              </div>
              ...
              <div className="flex flex-col gap-8 md:justify-center">
                <div>
                  ...
                  <p className="font-medium text-white">{t('whatYouBook.feature1', locale)}</p>
                  <p className="mt-1 text-sm leading-relaxed text-white/50">
                    {t('whatYouBook.feature1Desc', locale)}
                  </p>
                </div>
                <div>
                  <span className="mb-3 block h-px w-8 bg-amber-400/50" />
                  <p className="font-medium text-white">{t('whatYouBook.feature2', locale)}</p>
                  <p className="mt-1 text-sm leading-relaxed text-white/50">
                    {t('whatYouBook.feature2Desc', locale)}
                  </p>
                </div>
                <div>
                  <span className="mb-3 block h-px w-8 bg-amber-400/50" />
                  <p className="font-medium text-white">{t('whatYouBook.feature3', locale)}</p>
                  <p className="mt-1 text-sm leading-relaxed text-white/50">
                    {t('whatYouBook.feature3Desc', locale)}
                  </p>
                </div>
              </div>
            </div>
        ...
        <Section background="white">
          <Reveal>
            <div className="mb-10 text-center">
              <h2 className="text-4xl font-bold text-foreground md:text-6xl text-balance">{t('faq.heading', locale)}</h2>
              <p className="mx-auto mt-3 max-w-lg text-muted">
                {t('faq.subtext', locale)}
              </p>
            </div>
            ...
            <div className="mt-8 text-center">
              <p className="mb-4 text-sm text-muted">{t('faq.cta', locale)}</p>
              <Button href="/faq" variant="outline" className="border-amber-400 text-amber-400 hover:bg-amber-50">
                {t('faq.viewAll', locale)} <ArrowRight className="h-4 w-4" />
              </Button>
            </div>
          </Reveal>
        </Section>
